...
| **P10.S06** | **Unauthenticated delivery, so a live relay stops learning the sender** (AUDIT 3.9, ACCEPTED with compensating controls at P9.S09 and owned here). Added 2026-08-12 to give 3.9 a scheduled home rather than leaving it unscheduled; it is **not** a licence to start it. Signal gates its anonymous send path on an unidentified-access key derived from the recipient's **profile key**, and Cipher has no profile keys and no way to hand a peer an access token before the first message — so this is a protocol design that must arrive **with** an abuse control, because without one it is an unauthenticated write to a stranger's queue. Related: **5.41** has the same root cause, that the session token is the only credential path, and a re-authentication design would serve both |
