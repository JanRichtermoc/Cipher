...
| Next planned feature | **P6.S01 is complete (2026-08-07)** — prekey rotation, threshold replenishment, and bounded retention of the pair each rotation replaces, closing AUDIT 2.4 and the 5.36 it uncovered. It needed no relay change, so nothing is waiting on a deploy. **P6.S02 is next**: with rate limiting (P4.S06) and rotation (P6.S01) both live, document the residual witness-eviction trade-off and only then move AUDIT 3.1 to CLOSED or formally ACCEPTED — a judgement to record, not code to write. Then P6.S03 disappearing messages and P6.S04 attachments. One operator decision still stands between here and a formal **P5** exit: a second real device for the two-device safety-number criterion, or an explicit recorded acceptance of the one-device-plus-simulator residual. Nothing in the repository can supply either. **5.35** separately carries foreground-only delivery, fixed in P8.S02 and not P7.S03. |
...
| 3.1 | Base-key witness FIFO eviction. **Both controls now exist** (P4.S06 rate limiting, P6.S01 rotation); what remains is the judgement, not the code | P6.S02 |
