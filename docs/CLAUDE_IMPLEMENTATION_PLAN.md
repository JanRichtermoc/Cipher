...
|---|---|
| Phase | **P6 in progress**; P1–P4 complete. P5's numbered steps are all done and six of its seven exit criteria are ticked; the seventh is a hardware decision, not engineering work, so P6 proceeds beside it rather than behind it. |
...
| Next planned feature | **P6.S01 and P6.S02 are complete (2026-08-07).** P6.S01 delivered prekey rotation, threshold replenishment and bounded retention, closing AUDIT 2.4 and the 5.36 it uncovered; it needed no relay change, so nothing waits on a deploy. P6.S02 resolved AUDIT 3.1 as **ACCEPTED** and corrected the premise the row rested on — witness eviction is a *send*-side cost, so the P4.S06 fetch limit bounds pool drain and not it. **P6.S03 is next**: disappearing messages that actually delete, across a restart and a backgrounding. Then P6.S04 attachments and P6.S05 block list / device revoke. **6.18 is newly OPEN** and is tooling rather than product: the account-destruction guard lives only in `verify-all.sh`, so a direct `xcodebuild test` bypasses it — it already destroyed the P5.S13 simulator installation. One operator decision still stands between here and a formal **P5** exit: a second real device for the two-device safety-number criterion, or an explicit recorded acceptance of the one-device-plus-simulator residual. Nothing in the repository can supply either. **5.35** separately carries foreground-only delivery, fixed in P8.S02 and not P7.S03. |
