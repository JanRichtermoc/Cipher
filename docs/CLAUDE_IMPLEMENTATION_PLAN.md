...
| ~~5.41~~ | ~~"Mass revoke" has no non-destructive form: no operator endpoint, and the SQL form is unrecoverable per account and erases each device's local history when it next rotates (opened by P9.S06)~~ | **CLOSED P9.S10 + P9.S11** (2026-08-13); **deployed to staging 2026-08-15** at `cdb69db`, with migration `0003` and the recovery/rate-limit invariants observed live (`RUNBOOK-VPS.md`, P9.S11 deploy) |
...
| ~~5.42~~ | ~~The per-account re-authentication limit was charged before a caller-supplied `aci` was authenticated, so failures could spend a chosen account's recovery budget~~ | **CLOSED 2026-08-14** — the operator approved charging it only after a valid signature; failures remain under the per-address limit, and integration tests pin both the denial fix and the surviving account ceiling |
...
- [ ] High findings closed or formally ACCEPTED with compensating controls — **P9.S09 resolved 3.8
      ...
      closure reached the staging relay on 2026-08-15**, where migration `0003`, the three routes,
      the failed-signature budget invariant, and the surviving account ceiling were observed live.
      This box is about findings; a production cutover still reads it alongside the staging evidence
      in `RUNBOOK-VPS.md` and repeats that verification on the separate production host
