...
| 5.41 | "Mass revoke" has no non-destructive form: no operator endpoint, and the SQL form is unrecoverable per account and erases each device's local history when it next rotates (opened by P9.S06) | unscheduled — needs an approved re-authentication design |
...
- [ ] High findings closed or formally ACCEPTED with compensating controls — **P9.S09 resolved 3.8
      ...
      It stays unticked on four rows with owners rather than dispositions: **5.41** (mass revoke has
      no non-destructive form — needs an approved re-authentication design, shares a root cause with
      P10.S06), **6.3** and **6.15** (legal determinations, P8.S06), **6.4** (hardening, P10.S04)
