...
|---|---|
| Phase | **P8 is in progress out of order**: S04 (no NSE), S05 (privacy-manifest re-check), S07 (acknowledgements) and S08 (no crash reporter) are done — every P8 step that does not need the operator, because S01 is a human step blocked on an Apple Developer account and S02/S03 need the key it produces. **Every numbered P7 step is done (S01, S02, S03).** Its exit criteria are met but the phase carries one new OPEN finding, 3.9. P6's numbered steps are all done; P1–P4 complete. P5's numbered steps are all done and six of its seven exit criteria are ticked; the seventh is a hardware decision, not engineering work, so P6 proceeds beside it rather than behind it. **Amended 2026-08-11 on the operator's explicit approval: P9's precondition is relaxed for the four steps that need neither the APNs key nor a production VPS.** S04, S05, S06 and S07 may run now against staging, and a new **S09** resolves 3.8 and 3.9 to a status the production gate can read. The amendment at the head of P9 owns the scope and, more importantly, what it deliberately does **not** cover. **P8 itself is unchanged and still cannot exit.** |
...
| Security-remediation queue | Empty of closed work: 5.26, 5.27, 1.14, 6.14, 6.13, 5.28, 5.29, 5.32, 5.33 and 5.34 are closed; 5.29 reached the live box 2026-08-06 and 5.32 on 2026-08-07. 5.34 and 6.17 are client- and tooling-only and need no deploy. **5.1 closed 2026-08-07 (P5.S14); 5.36 was opened and closed by P6.S01 the same day.** That sentence used to end "and no 5.x row is OPEN", which the audit campaign made untrue on 2026-08-09 by opening **5.39** and **5.40**; **5.39 closed 2026-08-10 and 5.40 closed 2026-08-11**, each as its own unit, so no 5.x row was OPEN again — and that sentence has now been made untrue a second time, in the same way and for the same reason it is worth writing down. **5.41 closed 2026-08-13** (P9.S10 + P9.S11) and review of that closure opened **5.42** the same day: the per-account re-authentication limit is keyed on an `aci` an unauthenticated caller supplies, so a chosen account's recovery budget can be spent by anyone who knows it. It is OPEN and unscheduled, because the candidate fix removes a control P9.S11 argues for and the trade is the operator's. **5.41's closure and 5.42's reachability both stop at the repository**: the staging relay is on `6e6bbf2`, without migration `0003` or the three routes, and `RUNBOOK-VPS.md` carries the pending `P9.S11 deploy` row. **Both reached staging on 2026-08-11**; the runbook's deploy row owns the evidence. Closing it split out the push clause it also carried as **5.35** (ACCEPTED, foreground-only delivery, fix in P8.S02) rather than dropping it. What remains in this ledger is ACCEPTED residuals, which are read, not cleared. `AUDIT.md` is authoritative, and a finding's CLOSED status describes the repository: check the deployed revision against `origin/main` before assuming the staging relay carries it (`RUNBOOK-VPS.md`, Deployed revision). **5.32 is why that check exists, and 5.33 is why it is not paranoia** — both passed CI while the relay a real iPhone was talking to did not have them. |
| Next planned feature | **P6.S01 through P6.S04 are complete.** P6.S01 delivered prekey rotation and threshold replenishment (AUDIT 2.4, and the 5.36 it uncovered); P6.S02 resolved AUDIT 3.1 as **ACCEPTED** after correcting the premise it rested on; P6.S03 (2026-08-08) delivered disappearing messages that delete rather than hide, with the timer carried on each message rather than held as conversation state; P6.S04 (2026-08-08) delivered attachments encrypted before upload with an AEAD and a ciphertext digest, an encrypted media cache inside the crypto container, and a derived wipe that covers every path a message can leave the archive by — which also made P6.S03's media clause testable, and it is now tested; P6.S05 (2026-08-08) found both halves of its own `Done when` already satisfied — the mock revoke went with `MockStore` in P5.S10 and blocking was already real and tested — and closed the one clause that was not, server-side session-token revocation, which was called but never asserted. **Every numbered P6 step is complete and all five exit criteria are ticked.** **P7.S01 (2026-08-08) sealed the sender**: every outbound frame is a `.sealed` container and carries seventeen zero bytes where the sender used to be, which closed **3.4** and split the part it never covered out as **3.9** — a live relay is still told who is sending by the bearer token, and closing that needs an unauthenticated delivery path that is not an approved step. The step also found that its own stated mechanism, a *server-issued* sender certificate, cannot be built here: the signature is XEd25519 over Curve25519 keys and the relay is Go with an iOS-only libsignal pin. The certificate is self-issued instead, with the operator's approval taken before any edit; **no relay change was made and no deploy is needed.** **P7.S02 (2026-08-08) bucketed the length**: nine sizes doubling from 256 bytes, applied to the plaintext rather than the ciphertext for the reason that row now records, so a frame's size no longer tracks what was written. Also client-only, also no deploy. **P7.S03 (2026-08-08) hardened the push token**, and found the table fully specified and entirely unimplemented — nothing in Go had ever touched it, so the property it was supposed to guarantee held only because no token could be stored at all. It is the first **relay** change of the phase: `internal/pushtoken`, `store/push.go`, a fifth sweep task, and **migration `0002`**, **deployed to staging 2026-08-08** and verified there — the table held 0 rows before and after, so narrowing the CHECK could not fail against existing data, and a second start reported `applied=0` (`RUNBOOK-VPS.md`, P7.S03 deploy row). `RELAY_PUSH_TOKEN_KEY` is optional and stays unset until P8; with no key the writes refuse rather than storing a token in the clear. **P8.S01 (the APNs key, a human step) is next**, and it gates P8.S02, which brings both the provider and the registration endpoint this step deliberately did not build. **6.18 is CLOSED (2026-08-08):** the account-destruction guard used to live only in `verify-all.sh`, so a direct `xcodebuild test` bypassed it and had already destroyed the P5.S13 simulator installation. `Keychain.shared` now returns a Keychain on a separate service while XCTest is loaded, in the shared handle rather than in the fixture, so it also covers the fixtures nobody has written yet. The *file* half is the stated residual: a test that opened the default container would still remove real files, which no test does and which gate 1 still refuses. One operator decision still stands between here and a formal **P5** exit: a second real device for the two-device safety-number criterion, or an explicit recorded acceptance of the one-device-plus-simulator residual. **5.35** separately carries foreground-only delivery, fixed in P8.S02 and not P7.S03. **P9.S07 is done (2026-08-11)**: the internal pen-test checklist, executed against the live staging relay and a real installation — all nine items held, no new finding, evidence in [`STEP_NOTES/P9.S07.md`](STEP_NOTES/P9.S07.md). **P9.S05 is done (2026-08-11)**: encrypted backups scoped to the two tables no client can repair, a gate that refuses an unclassified schema table, and a restore drill executed on staging into a scratch database — which opened and accepted **4.16**, the revocation a restore undoes. **P9.S06 is done (2026-08-11)**: the incident-response runbook, which found that "mass revoke" — one of its own four named items — has no non-destructive form and opened **5.41** for it. **P9.S04 is done (2026-08-12)**: host-side monitoring that reports counts rather than a scrapeable per-delivery counter, whose live drill found an R2 defect in its own access-log parser. **P9.S09 is done (2026-08-12)**: 3.8 and 3.9 are ACCEPTED with tested compensating controls, and 3.9 gained an owner in the new P10.S06. **Every P9 step that needs neither the operator nor the production VPS is now complete** — S04, S05, S06, S07 and S09. What remains in P9 is S01/S02/S03 (production VPS, operator), S08 (external engagement, operator), and the production-readiness gate, which cannot be fully ticked while 6.3 and 6.15 await a legal determination and 6.4 is unscheduled hardening. **P9.S10 and P9.S11 are done (2026-08-13)**, the two halves of the approved re-authentication proposal: an ended session no longer erases the device, and an Ed25519 account key lets it come back — which **closes 5.41**. The next unit is an operator decision, not an engineering one: the remaining P9 work is the production VPS (S01/S02/S03) and the external engagement (S08), and P9.S11 needs a **staging deploy** for migration `0003`. |
| Audit campaign 2026-08-09 | A cross-cutting audit of everything already implemented, run as one branch and one pull request rather than a roadmap step, on the operator's explicit authorization. It changed no roadmap state and pulled no future work forward. It opened and closed **2.6, 5.37, 5.38, 6.19, 6.20, 6.21, 6.22, 6.23**, opened **5.39** and **5.40** as OPEN because both need a relay change and a deploy, and amended **2.4, 3.8, 4.4**. **5.39 closed 2026-08-10** as its own unit: a per-recipient ceiling on pending envelope bytes, enforced inside `EnqueueMessage`'s existing statement, accepted-and-dropped so it cannot become an oracle about the recipient. It is a **relay** change and the second one of this era to need a deploy. **5.40 closed 2026-08-11** as its own unit, as intended: a cumulative per-account ceiling on each one-time prekey pool, clipping the excess rather than refusing the publication, because the long-lived keys in the same request are what rotation replaces. **The campaign's OPEN queue is now empty**, and both closures reached staging on 2026-08-11 in one deploy, which also shipped AUDIT 6.24's compose wiring — undeployed since it was written, and found only by counting `server/` commits behind the box. `AUDIT.md` owns the detail; the one worth knowing here is **2.6** — no one-time Kyber prekey had ever been deleted after use, so the pool delivered none of the forward secrecy it exists for. **`verify-relay-integration.sh` did not run: Docker was unavailable**, so 6.20's guard was argued rather than observed — resolved 2026-08-10, the CI `relay-integration` job has since run it green three times. |
...
|-------|------|-----------|
| 3.8 | First-contact address is relabellable — narrowed by P7.S01, and again on 2026-08-10 when the addressed **first** message was refused outright. What remains is the self-issued certificate: a circle member can still name another account, in the ciphertext as well as the certificate | **ACCEPTED P9.S09** — no fix that does not contradict §0.2.1 |
| 3.9 | The live relay still learns the sender from the bearer token on send (split from 3.4 when P7.S01 closed it) | **ACCEPTED P9.S09** — the threat model already carried it as a residual; owned by P10.S06 |
| 4.4 | NSE/App Group sharing is not approved or cross-process tested; Keychain sharing still needs a migration and rollback design | **ACCEPTED P8.S04** — by withdrawing the requirement, not by meeting it: wake-only push decrypts in the app's own process, so no NSE is on the path to notifications. The prohibition on adding one stands, backed by `verify-app-target-manifest.sh` |
...
| ~~5.41~~ | ~~"Mass revoke" has no non-destructive form: no operator endpoint, and the SQL form is unrecoverable per account and erases each device's local history when it next rotates (opened by P9.S06)~~ | **CLOSED P9.S10 + P9.S11** (2026-08-13). **Repository only — the staging relay does not have it**: the box is on `6e6bbf2`, without migration `0003`, so there the cost is unchanged. `RUNBOOK-VPS.md` carries the pending deploy row |
| 4.16 | A restore resurrects session tokens revoked since the backup (opened and ACCEPTED by P9.S05) | ACCEPTED; re-argue at P9.S01 |
| ~~6.2~~ | ~~Acknowledgements not in About screen~~ | CLOSED P8.S07 — `verify-acknowledgements.sh` fails on drift from the pod's generated file |
| 5.42 | The per-account re-authentication limit is keyed on an `aci` an unauthenticated caller supplies, so ten requests an hour from one address spend a chosen account's recovery budget — the denial 5.41's own control exists to prevent (opened 2026-08-13 by review of P9.S11) | unscheduled — the candidate fix removes a control P9.S11 argues for, so the trade is the operator's |
| 6.3 | `ITSAppUsesNonExemptEncryption` undeclared | P8.S06 (legal) |
| 6.4 | No jailbreak/debugger detection, no screenshot protection — hardening rather than controls, and none of it stops an attacker who already has code execution | P10.S04 (unscheduled) |
| 6.15 | The privacy manifest declares required-reason APIs but no collected-data categories, while the relay necessarily processes account identifiers, encrypted payloads and delivery metadata — a different obligation from required-reason disclosure | P8.S06 (legal), before distribution |
...
| **P8.S05** | Re-verify the `PrivacyInfo.xcprivacy` enumeration against whatever libsignal version ships. P1.S11 closed 6.1 outright, so this is a re-check, not unfinished work — and `verify-privacy-manifest.sh` runs on every build, so a bump that introduces a category has already failed the build by now. Confirm the two judgement calls in `docs/PRIVACY_MANIFEST.md` still hold: the `C617.1` reason (breaks if anything ever hands libsignal a path outside our container) and `clock_gettime`/`gettimeofday` still being off Apple's list. **DONE 2026-08-08.** The strongest answer available: `PINS.env` still pins `v0.99.1` / `97801d22`, byte for byte the version P1.S11 enumerated, so the July findings describe the current bundle rather than a predecessor — and the Swift source grep stayed un-re-run because the note conditions it on a bump. Both judgement calls were re-checked rather than restated. The `C617.1` one holds *only while Cipher never hands libsignal a path*, so that is what was tested: every file importing `LibSignalClient` searched for `URL`, `contentsOf`, `.path` and `FileManager`, and every hit is Cipher's own container URL or a `Data` byte append. **P6.S04 was the likeliest breaker and is not one** — `AttachmentCipher` imports CryptoKit, so attachment paths never approach libsignal. The `clock_gettime`/`gettimeofday` one was checked against Apple's live page, which **turned out to have been restructured**: the per-category API lists moved off the article and onto `NSPrivacyAccessedAPIType`. That is worth recording, because a future re-check reading only the old article finds no lists at all and could read that as the requirement lapsing. On the current page all five categories and every symbol the enumeration reasoned about are still listed, and neither of those two appears anywhere on it — a whole-document match, with a control in both directions. `verify-privacy-manifest.sh` exits 0 against `Release-iphoneos/Cipher.app`. | AI | 6.1 (re-check) | `Scripts/verify-privacy-manifest.sh` exits 0 against the release build, and both judgement calls are re-stated as still true in `PRIVACY_MANIFEST.md` | Assume last year's list |
...
| **P8.S08** | Crash reporting: none, or a redacting pipeline that provably cannot upload plaintext or secrets. **DONE 2026-08-08 — decided: none.** The redacting-pipeline option was considered and rejected rather than skipped: a crash report is a stack walk plus register and heap state, and this process holds the identity private key, the record key, decrypted bodies and session tokens in memory by design, so "provably cannot capture plaintext" would mean proving that across every future version of somebody else's SDK. The device-side `os_log` Cipher already has costs a five-person circle a debugging convenience instead. **Verified rather than asserted:** the Release bundle's `Frameworks/` holds exactly `CipherCrypto` and `LibSignalClient`; `otool -L` across all three Mach-O binaries links nothing outside Apple's frameworks, the Swift runtime and those two; no first-party source references a reporter SDK, `NSSetUncaughtExceptionHandler`, `sigaction` or MetricKit; and `NSPrivacyTracking` is `false` with no tracking domains. **Enforced at the choke point:** `verify-supply-chain.sh` now refuses a `Podfile` declaring anything but LibSignalClient — the same check discharges §4.1 and §4.4, because recognising reporter SDKs by name is a list that ages while refusing every dependency does not. Its self-test grew from 8 cases to 10, including the empty-Podfile case so "found nothing forbidden" cannot be the same verdict as "read nothing". | AI | — | Decision recorded; no plaintext-capable reporter linked | Add a default SDK |
...
**Preconditions:** P8 exit criteria met — **relaxed 2026-08-11 for four steps; see the amendment
directly below.**

**Amendment, 2026-08-11, on the operator's explicit approval.** P8 cannot exit: **P8.S01** needs an
Apple Developer account that does not exist yet, **S02** and **S03** need the APNs key it produces,
and **S06** is a legal determination. Serializing the whole of P9 behind that leaves four steps —
**S04, S05, S06 and S07** — waiting on a purchase none of them depends on, and two of them are
unticked items on the production-readiness gate below. They may therefore run **now, against
staging, out of order**. This is the move P8 itself already made when S04, S05, S07 and S08 went
ahead of a blocked S01; it pulls no future work forward, because these rows were already written.

**What the relaxation does not cover**, stated so a staging half is never mistaken for a finished
step: **P9.S01/S02/S03** still need the production VPS and the operator. The production half of S04
and S05 — prod alerting, prod backups — is *not* done when the staging half is, and each row says
so. And no relaxation makes the gate's `Two-device E2E on production`, `Safety numbers verified` or
`APNs content-free verified` items reachable; those stay blocked on hardware, on a second real
device, and on the Apple Developer account. Nothing here changes a locked decision, a finding
status, or the phase order of anything else.

**P9.S07 additionally needs the operator's authorization at run time.** Its adversarial items —
invite brute force, prekey-pool exhaustion, base-key witness flooding — put deliberate load on a
live staging relay, so they can trip the rate limiter and the per-recipient and per-account ceilings
that AUDIT 5.39 and 5.40 added, and can degrade staging availability. Rehearse against local Docker
first, and do not start the live half without that authorization.
...
| **P9.S04** | Monitoring: metrics and alerts **without message content** — cert expiry, disk, auth anomalies, error rates. **Staging half unblocked 2026-08-11** by the amendment above. Doing it on staging does not do it on production: prod alerting ships with the P9.S02 deploy and stays blocked. **DONE 2026-08-12**, staging half; evidence in [`STEP_NOTES/P9.S04.md`](STEP_NOTES/P9.S04.md). `server/deploy/monitor.sh` checks certificate expiry, disk, container health, hourly error count and aggregate auth/throttle counts, and exits 0/1/2 — **not a `/metrics` endpoint**, because a counter that increments once per delivery is the delivery record §7 forbids, retained and scrapeable rather than rotated. Its self-test runs inside gate 10 and fails if any log content reaches an alert. **Running it against the live box found three defects that had already passed a self-test**, one of them AUDIT **R2** inside this step: the access-log parser matched nothing against nginx's `minimal` format and reported a confident `401=0`, so it now returns `parsed` alongside `total` and refuses to report a rate it could not compute. Alerts fired in two drills: every threshold forced (exit 2, CRITICAL) and an unparseable log (exit 1, WARN). | AI | Alerts fire in a drill | Log message metadata; call the staging half done for production |
| **P9.S05** | Encrypted server backups + a restore drill. Define RPO/RTO and account for message ciphertext, public key material, metadata, and operational server secrets separately. Backups inherit the §3.1 retention rules. **Staging drill unblocked 2026-08-11** by the amendment above, and it is the gate's `Backup restore drill done` item. The production backup set is separate and stays blocked with P9.S02. **ACCEPTED 4.8 bounds what any of this can promise**: the provider's whole-disk snapshot retains what the relay deletes, for up to 24 hours, and cannot be turned off — a backup design that ignores it would be describing a retention guarantee the host does not honour. **DONE 2026-08-11**, staging half; evidence in [`STEP_NOTES/P9.S05.md`](STEP_NOTES/P9.S05.md). The backup set is `accounts` and `session_tokens` and nothing else — §4's rule had no mechanism under it, so it was true only because no backup tooling existed. `server/deploy/backup.sh` is an allow-list encrypted with `openssl cms` to a recipient certificate, so **the host cannot decrypt its own backups**; `Scripts/verify-backup-scope.sh` is gate **11** and fails if any schema table is classified as neither backed up nor excluded, so a future migration fails closed. **Drill executed on the box into a scratch database**, live database read-only: 5 accounts and 3 tokens restored exactly, and messages, attachments, invites, one-time prekeys and push tokens all **0**. Including `session_tokens` is unavoidable — it is the only credential path — and its cost is opened and ACCEPTED as **AUDIT 4.16**. RPO 24h / RTO ~1h are in `INFRASTRUCTURE.md`. | AI | Restore drill completed | Back up what should have been deleted |
| **P9.S06** | Incident response runbook: compromise, pin rotation, mass revoke, key rotation. **Unblocked 2026-08-11** by the amendment above — it is documentation and touches no host, so nothing in it ever waited on a purchase. **DONE 2026-08-11**: [`RUNBOOK-INCIDENT.md`](RUNBOOK-INCIDENT.md), six incident classes with contain/preserve/eradicate/recover/notify, routing to the owning procedure rather than restating it — `BACKEND.md` §9.1 already owns pin and key rotation, so this indexes it instead of shipping a second copy to go stale. **Writing it found that one of the row's own four items does not exist**, the fourth step in this stretch to find its premise wrong: "mass revoke" has no non-destructive form, opened as **AUDIT 5.41**. `DELETE /v1/auth/all` is per-account and self-service, the only operator action is a SQL delete, and because the session token is the sole credential path that action is unrecoverable per account *and* silently erases each device's local history up to 23 days later when it next attempts rotation. Documented as a last resort with its true cost, with the three non-destructive containment steps ordered ahead of it, rather than listed as a routine control. | AI | Runbook exists and is walkable | — |
| **P9.S07** | **Internal pen-test checklist** (inlined here — the cited RTF §15.1 does not exist): `UserDefaults` bypass attempts; MITM with the wrong pin; relay rewriting `Envelope.sender`; relay replaying a captured prekey message; prekey pool exhaustion; invite brute force; base-key witness flooding; post-first-unlock device extraction; verifying delivered messages are actually gone from the server. **Unblocked 2026-08-11** by the amendment above, and it is the highest-security-value unit currently reachable. **Several items already have repository coverage** — `server/internal/integration/adversarial_integration_test.go`, `SealedSenderTests`, `RelayTransportTests`, `InviteRedemptionTests` — so the unit's value is executing them against a **live relay and a real installation**, which is where 5.32 and 5.33 were found and where a passing test proved nothing. Record what each item was executed against; an item discharged only by citing a unit test is not executed. **Needs the operator's run-time authorization** for the adversarial half (see the amendment). **DONE 2026-08-11** with the operator's authorization: all nine items executed and recorded in [`STEP_NOTES/P9.S07.md`](STEP_NOTES/P9.S07.md), each naming its substrate — client XCTest, the containerised relay suite, and live probes against the deployed staging relay (`6e6bbf2`, 0 `server/` commits behind `main`). Live evidence: TLS 1.3-only with the served leaf matching the shipped pin; invite redemption throttled to 5/hour through nginx with no unknown-vs-used oracle; the fetch route's pool-drain throttle firing after ten requests; and a self-send round-trip whose message row went 1 → 0 in a read-only `SELECT` after ack — actually deleted, not hidden. **No new finding: every control held.** The checklist lands on three residuals it does not close — AUDIT 3.9 (out of scope, owned by P9.S09), the ACCEPTED post-first-unlock extractability of 2.1, and the known 6.14 backup-key pin gap. | AI | Every item executed and recorded, each naming what it ran against | Skip an item as "obviously fine"; discharge a live item by citing a unit test |
...
| **P9.S10** | **Stop an ended session from erasing the device** (AUDIT 5.41, Change D of [`PROPOSAL-REAUTHENTICATION.md`](PROPOSAL-REAUTHENTICATION.md)). Added 2026-08-13 on the operator's approval of that proposal. A rejected rotation ran `signOut()`, which arms the destructive gate, so an operator-side revoke erased each device's protocol state, history and profile the next time rotation fell due — up to 23 days later, with nothing on screen linking the two. Client-only: no migration, no relay change, no deploy. **It is harm reduction, not a fix** — the account is still unrecoverable, which is P9.S11's job. | AI | A rejected rotation and an expired credential both leave the app signed out with nothing erased, erasure needs an explicit confirmation, and another invite is still unreachable until it happens | Erase without asking; make erasure unreachable |
| **P9.S11** | **An Ed25519 account key, so a device can re-authenticate** (AUDIT 5.41, Change A of [`PROPOSAL-REAUTHENTICATION.md`](PROPOSAL-REAUTHENTICATION.md)). Added 2026-08-13 on the operator's approval, with option **A1** — the revocation residual is accepted rather than answered with an operator freeze flag, whose abuse case is worse than what it prevents. Migration `0003` adds a nullable public-key column; `PUT /v1/auth/key` publishes write-once; `POST /v1/auth/challenge` and `POST /v1/auth/reauth` are unauthenticated by necessity and guarded by a signature over a server-chosen single-use challenge plus limits on both address and account. **Relay and client together**, because P7.S03 already recorded that building the half with no client is the wrong move. | AI | An account whose sessions are all deleted can obtain a working token by signing a challenge, replay and cross-account use are refused, and the key cannot be replaced | Let the relay hold any private key material; make the key replaceable |
| **P9.S09** | **Resolve AUDIT 3.8 and 3.9 to a status the gate can read.** Added 2026-08-11 with the amendment above. Both are **OPEN**, and both rows already argue that no code change is available or wanted: 3.8's remaining residual would need refusing an identity change, which contradicts locked decision §0.2.1, and 3.9 needs an unauthenticated delivery path that is deliberately unscheduled and has no abuse control behind it. The gate's second item asks for High findings *closed or formally ACCEPTED with compensating controls*, so while both sit OPEN with "no change wanted" attached, that item cannot be evaluated honestly in either direction — which is the failure this ledger exists to prevent. Either formally ACCEPT each with its compensating controls named, or schedule 3.9 as a real step with an approved design. **DONE 2026-08-12.** Both are now **ACCEPTED** with their compensating controls named and tested, and neither is closed by argument. **3.8**: every available fix contradicts something load-bearing — refusing an identity change contradicts §0.2.1, and verifying the certificate's name needs the server-issued scheme P7.S01 proved unbuildable here — and what remains is a §1.8 circle member forcing a **loud** identity-change event that clears the verified badge and blocks sending until the exact key is accepted. **3.9**: the ledger was the last document still calling it OPEN — `THREAT_MODEL.md` §1.1's residual row and §3.2 already carried it as an accepted exception, so the two disagreed about the same fact. Its compensating controls are the observation-versus-record distinction the threat model draws, and the record side is enforced and tested. It is now owned by the new **P10.S06** rather than left unscheduled. **The production-gate box is deliberately still unticked**: 5.41, 6.3, 6.4 and 6.15 remain, and this step's scope was these two rows. | AI | Neither row is OPEN with no owner: each is ACCEPTED with its compensating controls named, or is cited by a scheduled step | Close a finding because no fix is convenient; downgrade either to "probably fine" |

**Production readiness gate** — all must be true. Each item names the step that discharges it, so an
unticked box points at work rather than at a mood:
- [ ] All Critical findings closed — §0.5 records C-01, C-02 and C-03 as closed; confirm against it
      rather than against this line
- [ ] High findings closed or formally ACCEPTED with compensating controls — **P9.S09 resolved 3.8
      and 3.9 to ACCEPTED (2026-08-12)**, so the box is now *evaluable*, which it was not before,
      and **P9.S10 + P9.S11 closed 5.41 (2026-08-13)**, which was the fourth row on this list.
      It stays unticked on three rows with owners rather than dispositions: **6.3** and **6.15**
      (legal determinations, P8.S06) and **6.4** (hardening, P10.S04, unscheduled). **5.41's
      closure is in the repository and not on the staging relay**, which has neither migration
      `0003` nor the three routes — this box is about findings, but a production cutover reads it
      alongside `RUNBOOK-VPS.md`'s pending `P9.S11 deploy` row, and the deploy is an operator
      action
- [ ] Two-device E2E on production — P9.S01/S02/S03, and the second real device P5's last exit
      criterion also waits on
- [ ] Safety numbers verified — same second device
- [ ] App lock with biometrics verified — P3.S02
- [ ] APNs content-free verified — P8.S01, then P8.S02
- [ ] CI supply-chain + tests green — derive from the current run, never from this line
- [x] Backup restore drill done — **P9.S05, 2026-08-11**, on staging into a scratch database: 5
      accounts and 3 session tokens restored exactly, every retention-excluded table back at 0, live
      database untouched. The production backup set is separate and stays with P9.S02.
- [ ] Pen-test criticals closed — internal checklist **P9.S07 done 2026-08-11 with no criticals found**; external engagement P9.S08 still pending
...
| **P10.S06** | **Unauthenticated delivery, so a live relay stops learning the sender** (AUDIT 3.9, ACCEPTED with compensating controls at P9.S09 and owned here). Added 2026-08-12 to give 3.9 a scheduled home rather than leaving it unscheduled; it is **not** a licence to start it. Signal gates its anonymous send path on an unidentified-access key derived from the recipient's **profile key**, and Cipher has no profile keys and no way to hand a peer an access token before the first message — so this is a protocol design that must arrive **with** an abuse control, because without one it is an unauthenticated write to a stranger's queue. Related but **not solved by the same design**, and this row said otherwise until 2026-08-13: **5.41** shares a *symptom* with this row — "the session token is the only credential" — and not a solution. A re-authentication path is still authentication, so every send would still carry a bearer token and the relay would still see sender and recipient in one request. Closing 3.9 needs delivery that is *unauthenticated*, which is a different design. See [`PROPOSAL-REAUTHENTICATION.md`](PROPOSAL-REAUTHENTICATION.md) §5 |
...
(`verify-all.sh`), P4.S08 (delete-on-delivery), P7.S03 (push-token hardening), P9.S09 (resolve the
status of AUDIT 3.8 and 3.9, added 2026-08-11 with the P9 precondition amendment).
