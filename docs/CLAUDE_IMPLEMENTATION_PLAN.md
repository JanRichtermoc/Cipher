...
| **P9.S04** | Monitoring: metrics and alerts **without message content** — cert expiry, disk, auth anomalies, error rates. **Staging half unblocked 2026-08-11** by the amendment above. Doing it on staging does not do it on production: prod alerting ships with the P9.S02 deploy and stays blocked. | AI | Alerts fire in a drill | Log message metadata; call the staging half done for production |
