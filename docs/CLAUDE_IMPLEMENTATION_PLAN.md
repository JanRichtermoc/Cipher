...
| Next planned feature | **P6.S01, P6.S02 and P6.S03 are complete.** P6.S01 delivered prekey rotation and threshold replenishment (AUDIT 2.4, and the 5.36 it uncovered); P6.S02 resolved AUDIT 3.1 as **ACCEPTED** after correcting the premise it rested on; P6.S03 (2026-08-08) delivered disappearing messages that delete rather than hide, with the timer carried on each message rather than held as conversation state. **P6.S04 is next**: attachment encrypt-before-upload with AEAD and integrity checks, and the encrypted media cache wiped on delete or disappear — which is also what makes P6.S03's media clause testable. Then P6.S05 block list / device revoke. **6.18 is OPEN** and is tooling rather than product: the account-destruction guard lives only in `verify-all.sh`, so a direct `xcodebuild test` bypasses it. One operator decision still stands between here and a formal **P5** exit: a second real device for the two-device safety-number criterion, or an explicit recorded acceptance of the one-device-plus-simulator residual. **5.35** separately carries foreground-only delivery, fixed in P8.S02 and not P7.S03. |
...
- [x] Disappearing deletion tested across relaunch — P6.S03, 2026-08-08.
      ...
      own. The **media** half of the row's `Done when` has nothing to test until P6.S04 builds a
      media cache; recorded as not-applicable rather than ticked.
