...
| Completed P5 product steps | P5.S01, P5.S02, P5.S05, P5.S06, P5.S08, P5.S09, P5.S10, P5.S11, and P5.S12. |
| Security-remediation queue | Empty. 5.26, 5.27, 1.14, 6.14, 6.13, 5.28, 5.29 and 5.32 are all closed; 5.29 on the live box 2026-08-06 and 5.32 on 2026-08-07. `AUDIT.md` is authoritative, and a finding's CLOSED status describes the repository — check the deployed revision against `origin/main` before assuming the staging relay carries it (`RUNBOOK-VPS.md`, Deployed revision). **5.32 is why that check exists: the fix passed CI while the relay a real iPhone was talking to did not have it.** |
| Next planned feature | **P5.S13 is started, not finished.** Its first staging run found 5.32 and stopped there; no message has crossed between two devices. The relay now carries the fix, so the run can resume. It will run as one real iPhone plus one simulator — an operator-accepted residual recorded on `AUDIT.md` 5.1, which names the three things that evidence cannot prove. Then P5.S14. |
...
| **P5.S13** | Two-device end-to-end test via staging: register, exchange prekeys, send/receive real ciphertext. **STARTED, NOT DONE.** The first run got a real device through invite redemption against staging and then hit AUDIT 5.32 — the relay refused every prekey publication a real client can send — so registration could never complete and no ciphertext was exchanged. 5.32 is closed in the repository; the run resumes once staging carries the fix (operator-approved deployment). `AUDIT.md` 5.1 records the two residuals the resumed run must still resolve: it was one device plus one simulator, which proves nothing about device-Keychain classes, biometric app lock, or a Release-signed install. | AI + HUMAN | — | Real message delivered device-to-device | Test only in the simulator; call it done on one device plus a simulator without recording the residual |
