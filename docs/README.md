...
| [`PROPOSAL-REAUTHENTICATION.md`](PROPOSAL-REAUTHENTICATION.md) | **Proposal only — not approved, not implemented.** A re-authentication design for AUDIT 5.41, the options weighed, and the decision it awaits. Authoritative for nothing. | Proposal | Deciding 5.41, or before designing anything touching session credentials. | Superseded once decided |
