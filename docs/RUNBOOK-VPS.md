...
| **P9.S11 deploy** | **pending — operator** | Migration `0003` (nullable 32-byte `accounts.reauth_key`, with a CHECK that rejects any other length) and the three routes AUDIT 5.41 closes on: `PUT /v1/auth/key`, `POST /v1/auth/challenge`, `POST /v1/auth/reauth`. **Not deployed, so 5.41 and 5.42's CLOSED statuses describe the repository and not this box** — the distinction 5.32 and 5.33 exist to keep, both of which passed CI while the relay a real iPhone was talking to did not have them. Until it runs, deleting `session_tokens` on staging still costs every account its `aci`. Two things make this deploy unusually safe to reason about and one makes it worth care: the column is nullable and additive so no existing row can fail the CHECK, and a second start reports `applied=0`; but the two unauthenticated routes are the first in the design, so the post-deploy check must include that `POST /v1/auth/reauth` with a wrong signature answers **401 with the same body** as an unknown account, ten wrong signatures naming one account do **not** make its next valid signature return 429, the eleventh valid signature does return **429 with `Retry-After`**, and nginx does not exempt either route from the address limits §5 gives them. Follow the shallow-clone procedure in the 5.32 deploy row, previewing the diff before switching. |
