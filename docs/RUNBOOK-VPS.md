...
| P7.S03 deploy | **pending** | Migration `0002` narrows `push_tokens.token_nonce` from 24 bytes to 12 (AES-GCM). It applies on the relay's next start, needs no data migration — the table has never held a row — and there is nothing to roll back to, because nothing reads or writes the column yet. `RELAY_PUSH_TOKEN_KEY` is **deliberately not set** by this deploy: push does not exist until P8, and with no key the push-token writes refuse rather than storing a token in the clear. Setting it belongs with P8.S02, alongside the endpoint that first writes a token. Procedure is the ordinary one — the shallow-clone form in the 5.32 row, then `docker compose up -d --build`. Verify afterwards: `schema up to date` gives way to `applied=1` naming `migrations/0002_push_token_nonce.sql` on the first start and `applied=0` on every start after, all three containers healthy, `/health` and `/health/ready` 200 locally and over TLS, `ss` showing `127.0.0.1:8080` only. |
