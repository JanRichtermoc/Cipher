...
| **P9.S11 deploy** | **done 2026-08-15** | Operator explicitly approved this staging deployment. **Preflight:** the box was clean at `6e6bbf2`; `server/.env` was untracked, mode 0600, and never read; 33 GB was free; all three containers were healthy; migrations `0001` and `0002` were present, `accounts.reauth_key` was absent, and the database held five accounts. The shallow clone fetched exact target `cdb69db`; the complete live-to-target preview contained 14 `server/` paths (1,560 insertions, 10 deletions), production `docker-compose.yml` was unchanged, `git diff --check` was clean, and the exact commit's GitHub `relay-integration` and full `verify` jobs passed before mutation. **Deploy:** `git checkout -B main FETCH_HEAD`, then `docker compose up -d --build api`. Only `api` was recreated, on image `sha256:2bffe477…6730ab` built at 19:59:12Z; Postgres and Redis retained their 2026-08-07 start times and volumes. Startup reported `applied=1` naming `migrations/0003_account_key.sql`; the column is nullable `bytea`, the CHECK is `reauth_key IS NULL OR octet_length(reauth_key) = 32`, all five existing rows remained NULL, and an attempted 31-byte value was rejected by that exact constraint without changing the count. An API-only restart on the same image reported `applied=0`. **Live protocol proof:** temporary Ed25519 keys and response bodies stayed in a 0700 tmpfs directory and no account id, key, token, response body, or database credential was printed. Through public TLS, first publication returned 204, identical re-publication 204, replacement 409; a known account with the wrong key and an unknown account both returned 401 with byte-identical bodies. A focused 5.42 control produced ten 401s for wrong signatures and then 200 for the owner's valid signature from another address. The positive control produced ten 200s for valid recoveries and 429 with a positive `Retry-After` on the eleventh from another address. Each synthetic account was cascade-deleted with its sessions; the final account count returned to five and no original account had a reauth key yet. Nginx has zero route-specific exceptions for the two unauthenticated paths. **Operator traps found and contained:** the first verifier let containerized `psql` consume the remaining SSH stdin after its synthetic insert. No probes ran; aggregate tuple metadata proved the newest transaction was exactly two synthetic rows plus one session and no message/prekey/push data, so exactly those two rows were deleted and the count returned 7 → 5 before retry. A later header assertion mistook curl's carriage return for a missing `Retry-After`; its synthetic account was also cleaned, and CR-normalized parsing then observed the positive header. **Final state:** checkout clean at `cdb69db`, zero behind; zero API ERROR lines since deploy; both health endpoints 200 locally and over verified HTTP/2; 8080 bound only to 127.0.0.1 and unreachable externally; Postgres/Redis have no host bindings; TLS 1.2 refused and TLS 1.3 accepted. `verify-pins.sh` validated chain, hostname, served-leaf membership in the shipped pins, certbot key reuse, and code/doc pin parity; as before, its full run exits non-zero only because the offline backup-key file was unavailable (AUDIT 6.14). Rollback remains `6e6bbf2` plus an API-only rebuild; migration `0003` is additive and the old binary ignores it. |
