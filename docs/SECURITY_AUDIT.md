...
|---|---|---|---|
| **M-05** | 1.11 | `golang.org/x/text` v0.29.0 → **v0.39.0** (the minimum fixed version, so the diff is the fix and nothing else; it carries `golang.org/x/sync` v0.21.0 because v0.39.0 requires it). Re-vendored. | **New gate:** `Scripts/verify-vulns.sh` runs pinned `govulncheck` as gate 5 of `verify-all.sh`, and again in the release job. Negative-tested by reverting the module. |
